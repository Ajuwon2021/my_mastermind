...
int main(int argument_counter, char** argument_values)
{
    ...
    char* const_code = intialize_flag_c(argument_counter, argument_values);
    printf("Will you find the secret code? \nPlease enter a valid guess");
    
    while(attempts < attempts_of_game) 
    {
        print_round(attempts + 1);
        char *guest_inp_code = get_input_cmd();
        if (check_buffer_for_valid(guest_inp_code)) 
        {
            printf("\nWrong input!\n");
            continue;
        }
        if (check_input_code(guest_inp_code)) 
        {
            printf("\nWrong input!\n");
            continue;
        }
        well_places_pieces_count = get_well_pieces_algo(guest_inp_code, const_code);
        if (well_places_pieces_count == 4) 
        {
            won_game();
        }
        miss_places_pieces_count = get_miss_places_algo(guest_inp_code, const_code);
